...
bot.command('/add', async (ctx) => {
  ...
  if(c.length < 3) {
    ...
    await axios.get(`${RIOT_API}/lol/summoner/v4/summoners/by-name/${encodeURI(c[2])}?api_key=${RIOT_KEY}`).then(response => {
      ...
      try {
        let data = fs.readFileSync('./profiles/profiles.json', 'utf8')
        ...
        if(data.find(i=>i.name===c[2])) {
          ...
          fs.writeFile('./profiles/profiles.json', JSON.stringify(data), function (err) {
            if (err) throw err;
...
bot.command('/ranks', async (ctx) => {
  ...
  try {
    const data = fs.readFileSync('./profiles/profiles.json', 'utf8')
...
bot.command('/notificationSend', async (ctx) => {
  ...
  try {
    if(ADMIN_ID === ctx.message.from_id) {
      const data = fs.readFileSync('./profiles/notifications.json', 'utf8')
...
bot.command('/notifications', async (ctx) => {
  ...
  try {
    ...
    fs.writeFile('./profiles/notifications.json', JSON.stringify(users), function (err) {
      if (err) throw err;
...
bot.command('/ans', async (ctx) => {
  ...
  try {
    ...
    if(c[1].toLocaleLowerCase() === quiz_ans.toLocaleLowerCase().split(' ')[0]){
      quiz_ans = undefined
    const data = fs.readFileSync('./profiles/quiz.json', 'utf8')
    let players = JSON.parse(data)
    ...
    fs.writeFile('./profiles/quiz.json', JSON.stringify(players), function (err) {
      if (err) throw err;
      console.log('file profiles saved!');
    });
...
bot.command('/anek', async (ctx) => {
  ...
  try {
    ...
    const data = fs.readFileSync('./anek/anekdots.json', 'utf8')
